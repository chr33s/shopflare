...
const envValue = String(args.values.env);
let envFile = `.env.${envValue}`;
try {
...
async function deploy() {
  try {
    const cloudflare = await $(/* sh */ `npm run deploy:cloudflare -- --env=${envValue}`);
    ...
    const shopify = await $(
      /* sh */ `npm run deploy:shopify -- --config=shopify.app.${envValue}.toml`,
    );
    if (shopify.code !== 0) {
...
async function server() {
  await $(/* sh */ `npm run dev -- --config=shopify.app.${envValue}.toml`, {
    stdio: "inherit",
  });
}
...
async function use() {
  console.log(`Using environment: ${envValue}\n`);
